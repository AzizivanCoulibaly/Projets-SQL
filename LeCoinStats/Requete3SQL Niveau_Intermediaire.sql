...
SELECT Nom, Prenom, SUM(MontantTotal) AS SommeVentes
FROM Employes
...
SELECT*, em.Nom AS NomEmployee, em.Prenom AS PrenomEmployee
FROM Ventes v
JOIN Employes em ON v.EmployeID = em.EmployeID;
...
SELECT Nom, Prenom, COUNT(VenteID) AS NbreVente
FROM Employes
LEFT JOIN Ventes USING(EmployeID)
GROUP BY Nom, Prenom;
...
SELECT NomFournisseur, Email, COUNT(ProduitID)
FROM Fournisseurs
...
SELECT Nom, Prenom, AVG(MontantTotal) AS MyeVte
FROM Clients
LEFT JOIN Ventes USING(ClientID)
GROUP BY Nom, Prenom; 
...
-- COALESCE
SELECT Nom, Prenom, coalesce(AVG(MontantTotal),0) AS MyeVte
FROM Clients
LEFT JOIN Ventes USING(ClientID)
GROUP BY Nom, Prenom; 
...
SELECT Nom, Prenom, COALESCE(SUM(MontantTotal),0) AS MtnAchat
FROM Clients
LEFT JOIN Ventes USING(ClientID)
GROUP BY Nom, Prenom;
...
CREATE VIEW Ca AS
SELECT ClientID, Nom, Prenom, YEAR(DateVente), SUM(MontantTotal) AS smmAchat
FROM Clients
LEFT JOIN Ventes USING (ClientID)
GROUP BY ClientID, Nom, Prenom, YEAR(DateVente);

-- Quelle est pour chaque client la moyenne des CA générés par année?
SELECT Nom, Prenom, YEAR(DateVente), AVG(MontantTotal) AS MyeAnné
FROM Ca
LEFT JOIN Ventes USING(ClientID)
GROUP BY Nom, Prenom, YEAR(DateVente);
...
-- Exercice: Écrire une requête qui permet de donner pour chaque client son nom, son prénom, 
-- la somme des achats ainsi que la moyenne annuelle des achats
SELECT Nom, Prenom, SUM(MontantTotal) AS CA, (SELECT AVG(MontantTotal) AS Moyenne_CA
											FROM Ventes)
FROM Clients
LEFT JOIN Ventes USING(ClientID)
GROUP BY Nom, Prenom
